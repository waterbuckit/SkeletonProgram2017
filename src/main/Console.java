 ...
 *
 * readChar and readByte use readLine and just take the first character entered,
 * which is converted into a char or a byte.
 *
 * readInteger uses readLine and converts the line into an integer. If this is
 * not possible then Integer.MIN_VALUE is returned as an indicator that it has
 * failed, an error message is also printed to the console.
 */
class Console {
     ...
     * with the return key, is returned. If there is an exception then a message
     * is written to the console and an empty string is returned.
     *
    ...
    /**
     * The parameter is output to the console and the response is converted to
     * an integer and returned
     *
     * @param prompt the output to the console
     * @return the response is converted to an integer before it is returned. If
     * there is an error then the minimum value an int can have is returned.
     */
